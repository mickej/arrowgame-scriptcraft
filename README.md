...
Typing `/js var b=self.world.getBlockAt(219,64,100)` gives me a variable `b` with the block I'm looking at. There will be no printout.

`/js b` shows details of the block. Try it on your own!

`/js b.typeId` gives "35", which is `blocks.wool.white`. But, since wool has a color we also need to check 
`/js b.data` which gives us "14". This is `blocks.wool.red`, which you can verify by typing `/js blocks.wool.red`

To turn off the debug screen, press F3 again.

## An arrow hits the target

Now we'll use events send between functions in a program. This is often used in Scratch. Taking this example,
